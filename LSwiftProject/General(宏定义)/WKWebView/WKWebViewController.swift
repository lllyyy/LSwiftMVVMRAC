...
class WKWebViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
      // self.urltring = "http://192.168.1.145:8888/test2.htm"
...
//        }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        guard self.isRefresh else {
            return
        }
      
        if let url = URL(string: urltring)  {
...
//        configuretion.userContentController.add(self, name: "callbackHandle")
        webView = WKWebView(frame:view.bounds)
        webView.top = kNavBarHeight
        webView.height = kScreenH - kNavBarHeight - CGFloat(tabHeight) 
        //开启手势交互
        webView.allowsBackForwardNavigationGestures = true
        webView.configuration.userContentController.add(self, name: "share")
        webView.configuration.userContentController.add(self, name: "toIntegral")
        webView.configuration.userContentController.add(self, name: "coupon")
        webView.configuration.userContentController.add(self, name: "shopping")
        webView.configuration.userContentController.add(self, name: "askdoctor")
        webView.configuration.userContentController.add(self, name: "changeNumber")
        webView.configuration.userContentController.add(self, name: "toDetail")
...
//        }else{
            progressView?.frame = CGRect(x: 0, y: kNavBarHeight, width: view.bounds.size.width, height: 3)
       // }
    ...
    override func viewDidDisappear(_ animated: Bool) {
       // webView.configuration.userContentController.removeScriptMessageHandler(forName: "toIntegral")
    ...
    @objc fileprivate func customBackItemClicked() {
        if (webView.goBack() != nil) {
            ...
            if self.backFlag == 0 {
                ...
                getTopVC(selectedIndex:  getTabVC().selectedIndex).navigationController?.popViewController( animated: true)
            }
    ...
    //KVO监听进度条变化
     override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if keyPath == "estimatedProgress"{
            ...
            self.customNavBar.title = self.webView.title
        }
    }
    ...
    fileprivate func updateNavigationItems(){
        ...
        if webView.canGoBack {
            ...
            self.customNavBar.wr_setLeftButton(with: UIImage.init(named: "menu_back"))
          
        }else{
 
           // self.customNavBar.leftButton.isHidden = true
...
//            self.share()
//        }else if (message.name == "toIntegral") {
//            self.pushIntegral()
//        }else if (message.name == "coupon") {
//            self.pushCoupons()
//        }else if (message.name == "shopping") {
//            self.shopping(message.body)
//        }else if (message.name == "askdoctor") {
//            self.askdoctor(message.body)
//        }else if (message.name == "changeNumber") {
//            self.changeNumber()
//        }else if (message.name == "toDetail") {
//            self.webView.goBack()
//            self.toDetail(message.body as! String)
//        }
   }
...
extension WKWebViewController: WKNavigationDelegate{
    ...
    func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
        ...
        if (newPath?.hasPrefix("tel:"))! {
            UIApplication.shared.openURL(  URL.init(string: newPath!)! )
        }
